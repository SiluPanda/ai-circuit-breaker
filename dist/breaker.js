...
function createBreaker(config) {
    ...
    const entries = config.budgets.map(b => ({
        window: b.window,
        tracker: (0, index_1.createWindowTracker)(b.window),
        spend: new spend_tracker_1.SpendTracker(b.limit, b.warningThreshold ?? 0.8, maxHistorySize),
    }));
    const machine = new state_machine_1.StateMachine(probeCount, cooldownMs);
    const fallback = new fallback_1.FallbackHandler(config.fallback ?? { strategy: 'throw' });
    let totalSpent = 0;
    /** Check all windows for expiry; reset expired ones and fire hooks. */
    function lazyWindowReset() {
    ...
    const breaker = {
        ...
        recordSpend(cost) {
            ...
            for (const entry of entries) {
                ...
                if (warningCrossed) {
                    const budgetDef = config.budgets.find(bd => JSON.stringify(bd.window) === JSON.stringify(entry.window));
        ...
        recordTokens(usage) {
            ...
            const cost = (usage.inputTokens * config.pricing.inputCostPer1M / 1_000_000) +
        ...
        reset() {
            ...
            if (previousState !== 'closed') {
                hooks.onClose?.({ previousState: previousState, totalSpent });
            }
        },
        addBudget(window, amount) {
            ...
            if (!entry)
                throw new TypeError('Window not found in configured budgets');
